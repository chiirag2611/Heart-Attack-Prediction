...
def add_bg_from_url():
    st.markdown(
         ...
         <style>
         .stApp {{
             background-image: url("https://lh6.googleusercontent.com/73eU2x6LPmul8ukmeDV1fP7p1aivkUZDGl5bxZ-yz4jaLwe1dG6G7xw5V-hEEjqKJqYqSgRqpatOrw1SZaOgCKU=w16383");
...
def header(url):
     st.markdown(f'<p style="color:#FF4B4B;font-size:46px;border-radius:2%;font-weight:bold">{url}</p>', unsafe_allow_html=True)

def subheader(url):
     st.markdown(f'<p style="color:#FF4B4B;font-size:36px;border-radius:2%;font-weight:bold">{url}</p>', unsafe_allow_html=True)


header("HEART FAILURE PREDICTION APP")
...
try:
    ...
    input_df['gender'] = np.where(input_df['gender'] == 1, 'Male','Female')
...
subheader('User Medical Profile')
...
subheader('Medical Profile Description')
